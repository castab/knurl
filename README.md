...
| `API_BEARER_TOKEN` | yes | presentation | Token required (as `Authorization: Bearer <token>`) on the public gallery GET endpoints and `POST /api/v1/accounts/{accountId}/galleries/{galleryId}/track`. |
...
| `INSTAGRAM_ACCESS_TOKEN` | database mode | ingestion | Long-lived Instagram Graph API access token used to seed standalone database-backed token refresh. |
...
| `INSTAGRAM_CREDENTIAL_PROVIDER` | no | ingestion | `DATABASE` (default) preserves standalone config/DB-backed token refresh; `HTTP_BROKER` obtains a valid token from an external credential broker. |
...
| `INSTAGRAM_CREDENTIAL_BROKER_TOKEN` | broker mode | ingestion | Account-scoped job bearer token used to authenticate to the broker. Configure this or the token-file option, never both. |
| `INSTAGRAM_CREDENTIAL_BROKER_TOKEN_FILE` | broker mode | ingestion | File containing the account-scoped job bearer token. Reread on every request to support rotating mounted workload credentials. |
...
**Token exposure:** `API_BEARER_TOKEN` guards browser-called gallery reads and `POST .../galleries/{galleryId}/track`, so any real browser client must ship it to the browser. Treat it as an access gate and rate-limiting speed bump against casual abuse, not strong authentication — it is not a secret once a gallery page uses it. A per-account `ADMIN_TOKEN` is a genuine secret: it is never embedded in a page, and the bundled UI keeps it only in `sessionStorage` for the life of the tab.

In standalone `DATABASE` credential mode, `INSTAGRAM_ACCESS_TOKEN` seeds the first refresh and refreshed values are stored in `auth_config`. In `HTTP_BROKER` mode, the broker owns refresh and persistence: ingestion requests a currently valid token at the start of each cycle, verifies that the response belongs to its configured account, and never reads or writes `auth_config`. Broker failures do not fall back to database credentials. The broker must authenticate each job as exactly one account and return JSON shaped as `{ "instagramAccountId": "...", "accessToken": "...", "expiresAt": "..." }` with `Cache-Control: no-store`; `expiresAt` must be an ISO-8601 instant more than 24 hours in the future.
...
> **Gallery names require the public bearer token.** `GET /api/v1/accounts/{accountId}/galleries` returns every gallery's name and item counts only to callers that provide `API_BEARER_TOKEN`. This token is intentionally less privileged than an account's admin token, but browser clients can still expose it.
